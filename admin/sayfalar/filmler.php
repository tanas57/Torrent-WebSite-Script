...
$icerik_sql			=	mysql_query("select * from filmler where copySeo='0' and durum!=2 and ($araAdi or $araIcrk)  group by adi order by id desc");
$i=1;
...
BaglantiAc();
$icerik_sql			=	mysql_query("select * from filmler where copySeo='0' and durum!=2 group by adi order by id desc limit 10");
$i=1;
		while($icerik_cek	=	mysql_fetch_array($icerik_sql))
			{
				$id			=	$icerik_cek["id"];
